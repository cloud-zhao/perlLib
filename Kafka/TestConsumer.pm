...
my $single_broker=sub{
	my $self=shift;
	my ($topic,$partition)=@_;
	my $topic_meta={$partition=>{
		Isr	=>	[0],
		Leader	=>	0,
		Replicas=>	[0]
	}};
	my $meta={$topic=>$topic_meta};
	my $conn=$self->{Connection};
	my $leaders={0=>$conn->_build_server_name($conn->{host},$conn->{port})};
	$conn->{_metadata}=$meta;
	$conn->{_leaders}=$leaders;
	return $self;
};
...
sub fetch {
    ...
    $self->$single_broker($topic,$partition);
